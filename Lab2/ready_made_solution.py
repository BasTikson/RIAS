...
view.view(gamma_m, 'gamma_m.html')
...
view.view(m_pred_num, 'm_pred_num.html')
...
view.view(m_pred_up, 'm_pred_up.html')
...
print('pred_down', result_pred_down)
...
view.view(m_pred_down, 'm_pred_down.html')
...
for i in max_common_array:
    if len(max_common_array[i]) > 1:
        ...
        print('что-то ', count, groups[count])
        count+=1
...
view.view(m_pred_group, 'm_pred_group.html')
...
m_pred_group = pd.DataFrame(data=groups.values(), index=groups.keys())
print(m_pred_group)
view.view(m_pred_group, 'm_pred_group2.html')